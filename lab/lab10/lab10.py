...
def calc_eval(exp):
    ...
    if isinstance(exp, Pair):
        operator = exp.map(calc_eval).first # UPDATE THIS FOR Q2, e.g (+ 1 2), + is the operator
        ...
        if operator == 'and': # and expressions
            ...
            return calc_apply(operator, operands.map(calc_eval)) # UPDATE THIS FOR Q2, what is type(operator)?
...
def eval_define(expressions):
    ...
    if expressions.rest.first in bindings:
        bindings[expressions.first] = bindings[expressions.rest.first]
    elif expressions.rest.first in OPERATORS:
        OPERATORS[expressions.first] = OPERATORS[expressions.rest.first]
    else:
        bindings[expressions.first] = expressions.rest.first
    return expressions.first

OPERATORS = { "//": floor_div, "+": addition, "-": subtraction, "*": multiplication, "/": division, "and": 'and', "define":'define' }

...
def balanced(m):
    ...
    """
    "*** YOUR CODE HERE ***"
    return (length(left(m)) * total_mass(end(left(m))) == length(right(m)) * total_mass(end(right(m)))) and (True if is_planet(end(left(m))) else balanced(end(left(m)))) and (True if is_planet(end(right(m))) else balanced(end(right(m))))
...
def berry_finder(t):
    ...
    for b in branches(t):
        if berry_finder(b):
            result = True
    return label(t) == 'berry' or result 
...
def path_sum(t, total):
    total += label(t)
    if is_leaf(t):
        return [total]
    else:
        return sum((path_sum(b, total) for b in branches(t)), [])

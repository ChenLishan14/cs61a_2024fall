...
def about(subject):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    def about_subject(strings):
        count = 0
        for word in split(remove_punctuation(strings)):
...
def memo_diff(diff_function):
    ...
    def memoized(typed, source, limit):
        ...
        if (immutable_args in cache) and (cache[immutable_args][1] >= limit):
            ...
            cache[immutable_args] = (diff_function(*immutable_args, limit), limit)
            return cache[immutable_args][0]
        # END PROBLEM EC
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for word in word_list:
        ...
        if diff <= limit:
            if diff < min or count == 0 or word == typed_word:
                min = 0 if word == typed_word else diff
                result = word
                count += 1
    return result
    # END PROBLEM 5
...
def furry_fixes(typed, source, limit):
    ...
    if length1 == 0 or length2 == 0:
        return min(abs(length1 - length2), limit + 1)
    else:
        change = 0 if typed[0] == source[0] else 1
        if limit == 0 and change:
            return change
        else:
            return change + furry_fixes(typed[1:], source[1:], limit - change)
    # END PROBLEM 6
...
def report_progress(typed, source, user_id, upload):
    ...
    length1 = len(typed)
    length2 = len(source)
    while i < length1 and typed[i] == source[i]:
        i += 1
    progress = i / length2
...
def fastest_words(words_and_times):
    ...
    for i in word_indices:
        ...
        for j in player_indices:
            if get_time(times, j, i) < min_time:
                min = j
                min_time = get_time(times, j, i)
        fast_word.append(min)

...
def roll_dice(num_rolls, dice=six_sided):
    ...
    while k < num_rolls:
        ...
        if outcome == 1:
            flag = 1
        result += outcome
    return result if flag == 0 else 1
    # END PROBLEM 1
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    num = 0
    while k <= 10:
        result = make_averaged(roll_dice, times_called)(k, dice)
        if result > max:
            max = result
            num = k
        k += 1
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    Suss Fuss rules."""
    # BEGIN PROBLEM 11
    result = sus_update(0, score, opponent_score, dice=six_sided)

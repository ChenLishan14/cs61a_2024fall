...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        result = self.place
        distance = 0
        while result.entrance != None and distance < self.lower_bound:
            result = result.entrance
            distance += 1

        while result.bees == [] and result.entrance != None and result.entrance.is_hive == False and distance < self.upper_bound :
            result = result.entrance
            distance += 1
        return random_bee(result.bees) # REPLACE THIS LINE
        # END Problem 3 and 4
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        result = self.place.exit
        while result:
            if result.ant != None:
                if result.ant.is_container and result.ant.ant_contained:
                    result.ant.ant_contained.double()
                result.ant.double()
            result = result.exit
        return super().action(gamestate)
        # END Problem 12
